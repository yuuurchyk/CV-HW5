...
# classification on CIFAR10, from embeddings of e9 without projection
EXP_NAME = "exp11"
EPOCHS_NUM = 2000
BATCH_SIZE = 4096
EMBEDDING_SIZE = 128
...
def main():
    ...
    encoder = nn.Sequential(vgg, projection)
    encoder = encoder.to(device)

    d = torch.load(os.path.join(get_exps_root(), 'exp9', '175.pth'))
    encoder.load_state_dict(d['model'])
    # don't use projection head
    encoder = encoder[0]
