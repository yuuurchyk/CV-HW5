...
# classification on CIFAR10, from embeddings of e14
EXP_NAME = "exp15"
EPOCHS_NUM = 4000
BATCH_SIZE = 4096
EMBEDDING_SIZE = 128
...
def main():
    ...
    d = torch.load(os.path.join(get_exps_root(), 'exp14', '2200.pth'))
    ...
    # original_train = Cifar10(True, get_augmentations([], random_resized_crop_size=32), device=torch.device('cpu'))
    # original_validation = Cifar10(False, get_augmentations([], random_resized_crop_size=32), device=torch.device('cpu'))

    original_train = Cifar10(True, None, device=torch.device('cpu'))
    original_validation = Cifar10(False, None, device=torch.device('cpu'))
